...
SAVE_DIR_20FPS_ish=./experiments/ish_20FPS
SAVE_DIR_100FPS_ish=./experiments/ish_100FPS
...
python motion_tokenizer_humanml3d_multi_discretizers.py train \
  ...
  --save_dir  $SAVE_DIR_20FPS_ish \
  --normalize \
  --bottleneck ish \
  --vocab_size 8192 --ish_bits 13 \
  --ish_noise_std 1.0 --ish_forward_hard_rate 0.5 \
  --epochs 500 --batch_size 64 --lr 1e-4 \
  --max_motion_len 64 --patch_len 1

python motion_tokenizer_humanml3d_multi_discretizers.py train \
  --data_root $DATA_ROOT_100FPS \
  --save_dir  $SAVE_DIR_100FPS_ish \
  --normalize \
  --bottleneck ish \
  --vocab_size 8192 --ish_bits 13 \
  --ish_noise_std 1.0 --ish_forward_hard_rate 0.5 \
...
python motion_tokenizer_humanml3d_multi_discretizers.py reconstruct \
  --data_root $DATA_ROOT_20FPS \
  --save_dir  $SAVE_DIR_20FPS_ish \
  --ckpt      $SAVE_DIR_20FPS_ish/checkpoints/ckpt_best.pt \
  --split test

python motion_tokenizer_humanml3d_multi_discretizers.py reconstruct \
  --data_root $DATA_ROOT_100FPS \
  --save_dir  $SAVE_DIR_100FPS_ish \
  --ckpt      $SAVE_DIR_100FPS_ish/checkpoints/ckpt_best.pt \
  --split test
...
python eval_recon_humanml3d_motiongpt_metrics.py \
  ...
  --data_root  $DATA_ROOT_20FPS \
  --recon_root $SAVE_DIR_20FPS_ish/recon_out/test \
  --split test \
  --t2m_path   $T2M_DIR \
...
python eval_recon_humanml3d_motiongpt_metrics.py \
  ...
  --cfg        $MOTIONGPT_DIR/configs/config_h3d_stage3.yaml \
  --data_root  $DATA_ROOT_100FPS \
  --recon_root $SAVE_DIR_100FPS_ish/recon_out/test \
  --split test \
  --t2m_path   $T2M_DIR \
  --meta_dir   $MOTIONGPT_DIR/assets/meta \
