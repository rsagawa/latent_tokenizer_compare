...
def _resolve_recon_dir(recon_dir: Optional[str], recon_root: Optional[str], split: str) -> Path:
    ...
    if cand.is_dir():
        return cand
    # fallback: maybe user passed the split folder directly
    if root.is_dir():
        return root
    raise FileNotFoundError(f"--recon_root not found: {root}")
...
def main() -> None:
    ...
    # Evaluate over split ids (no segment keyids)
    ...
    for mid in ids:
        ...
        if gt_raw.ndim != 2 or pred_raw.ndim != 2 or gt_raw.shape[1] != 263 or pred_raw.shape[1] != 263:
            skipped_shape += 1
            if not args.quiet:
                print(f"[WARN] Skip {mid}: bad shape gt={gt_raw.shape} pred={pred_raw.shape}")
            continue
        ...
        if (np.isnan(gt_raw_ref).any() or np.isnan(pred_raw_rst).any() or
            ...
            skipped_nan += 1
            if not args.quiet:
                print(f"[WARN] Skip {mid}: NaN found")
            continue
