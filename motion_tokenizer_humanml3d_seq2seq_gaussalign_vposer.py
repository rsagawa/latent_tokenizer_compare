...
def _ensure_humanml3d_import_path() -> None:
    """Add ../HumanML3D to sys.path when local human_body_prior is vendored there."""
...
class VPoserSMPLJointBridge(nn.Module):
    """
    Convert vposer latent [*,32] -> SMPL-H Jtr [*,J,3].
    VPoser/BodyModel are frozen by excluding them from optimizer updates.
    """
    def __init__(
        self,
        *,
        ...
        self.latent_dim = int(getattr(vposer, "latentD", 32))
        self.register_buffer("zero_root_orient", torch.zeros((1, 3), dtype=torch.float32), persistent=False)
        self.register_buffer("zero_pose_hand", torch.zeros((1, 90), dtype=torch.float32), persistent=False)
        self.register_buffer("zero_trans", torch.zeros((1, 3), dtype=torch.float32), persistent=False)
        self.register_buffer("zero_betas", torch.zeros((1, int(num_betas)), dtype=torch.float32), persistent=False)

    def decode_latent_to_jtr(self, z_flat: torch.Tensor) -> torch.Tensor:
        """
        z_flat: [N,latent_dim]
        returns: [N,J,3]
        """
        if z_flat.numel() == 0:
            return z_flat.new_zeros((0, 0, 3))
        dec = self.vposer.decode(z_flat)
        if isinstance(dec, dict):
            pose_body = dec["pose_body"]  # [N,21,3]
        else:
            pose_body = dec
        pose_body = pose_body.reshape(z_flat.shape[0], -1).to(dtype=z_flat.dtype)
        n = pose_body.shape[0]
        body = self.body_model(
            root_orient=self.zero_root_orient.expand(n, -1).to(dtype=z_flat.dtype),
            pose_body=pose_body,
            pose_hand=self.zero_pose_hand.expand(n, -1).to(dtype=z_flat.dtype),
            trans=self.zero_trans.expand(n, -1).to(dtype=z_flat.dtype),
            betas=self.zero_betas.expand(n, -1).to(dtype=z_flat.dtype),
        )
        return body.Jtr


def recon_loss_vposer_jtr(
    ...
    *,
    mean: torch.Tensor,
    std: torch.Tensor,
    bridge: VPoserSMPLJointBridge,
    root_w: float = 1.0,
    joint_w: float = 1.0,
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    Reconstruction loss for 36D feature:
      - root4: direct masked MSE
      - vposer32: decode -> SMPL-H Jtr and compare in joint space
    """
    if pred_norm.shape[-1] != 36 or target_norm.shape[-1] != 36:
        raise ValueError(f"Expected 36D inputs, got pred={pred_norm.shape} target={target_norm.shape}")

    mean_b = mean.view(1, 1, -1).to(dtype=pred_norm.dtype, device=pred_norm.device)
    std_b = std.view(1, 1, -1).to(dtype=pred_norm.dtype, device=pred_norm.device)
    pred = pred_norm * (std_b + 1e-8) + mean_b
    target = target_norm * (std_b + 1e-8) + mean_b

    root_pred = pred[..., :4]
    root_tgt = target[..., :4]
    loss_root = masked_mse(root_pred, root_tgt, mask)

    z_pred = pred[..., 4:36]
    z_tgt = target[..., 4:36]
    B, T, Dz = z_pred.shape
    if Dz != int(bridge.latent_dim):
        raise ValueError(f"VPoser latent dim mismatch: data={Dz}, model={bridge.latent_dim}")
    z_pred_flat = z_pred.reshape(B * T, Dz)
    z_tgt_flat = z_tgt.reshape(B * T, Dz)
    jtr_pred = bridge.decode_latent_to_jtr(z_pred_flat)
    jtr_tgt = bridge.decode_latent_to_jtr(z_tgt_flat)
    if jtr_pred.numel() == 0 or jtr_tgt.numel() == 0:
        loss_jtr = pred_norm.new_zeros(())
    else:
        J = jtr_pred.shape[1]
        jtr_pred_bt = jtr_pred.reshape(B, T, J * 3)
        jtr_tgt_bt = jtr_tgt.reshape(B, T, J * 3)
        loss_jtr = masked_mse(jtr_pred_bt, jtr_tgt_bt, mask)

    total = float(root_w) * loss_root + float(joint_w) * loss_jtr
    return total, loss_root, loss_jtr
...
def list_motion_files(data_root: Path, split: str) -> List[Path]:
    ...
    Layout B (MotionGPT-like):
      data_root/
        new_joint_vecs/*.npy or new_vposer_root_vecs/*.npy
    ...
    # B) data_root contains known feature dirs (e.g., new_joint_vecs / new_vposer_root_vecs)
    # for subdir in ["new_joint_vecs", "new_vposer_root_vecs"]:
    for subdir in ["new_vposer_root_vecs"]:
        feat_dir = data_root / subdir
        if not feat_dir.exists():
            continue
        list_path = data_root / f"{split}.txt"
        if list_path.exists():
            ids = _read_split_ids(list_path)
            files = [feat_dir / f"{i}.npy" for i in ids if (feat_dir / f"{i}.npy").exists()]
            if len(files) > 0:
                return files
        # some datasets place split lists inside feature dir
        list_path2 = feat_dir / f"{split}.txt"
    ...
    raise FileNotFoundError(
        ...
        f"Tried: {data_root}/{split}/*.npy, "
        f"{data_root}/new_joint_vecs/*.npy (+ split txt), "
        f"{data_root}/new_vposer_root_vecs/*.npy (+ split txt), "
...
class TrainConfig:
    ...
    # vposer->SMPL-H joint-space reconstruction (for 36D root4+vposer32)
    vposer_expr_dir: str = "../HumanML3D/human_body_prior/train/V02_05"
    body_models_root: str = "../HumanML3D/body_models"
    body_model_gender: str = "male"
    body_model_num_betas: int = 10
    body_model_num_dmpls: int = 8
    rec_root_w: float = 1.0
    rec_joint_w: float = 1.0
...
def build_model_and_stats(cfg: TrainConfig) -> Tuple[MotionSeq2SeqARAE, np.ndarray, np.ndarray]:
    ...
    # infer/check feature dim
    x0 = np.load(train_files[0])
    feat_dim = int(x0.shape[1])
    if feat_dim != int(cfg.feat_dim):
        raise ValueError(
            f"Expected feature dim={cfg.feat_dim} (root4+vposer32), "
            f"but got {feat_dim} from {train_files[0]}"
        )
...
def _encode_chunk_incremental(
    ...
    x_new: torch.Tensor,
    keep_new: torch.Tensor,
    past_layer_states: Optional[List[torch.Tensor]],
    keep_past: Optional[torch.Tensor],
    *,
    ...
    for li, layer in enumerate(layers):
        ...
        q = layer.norm1(x)
        kv = q if past_l is None else torch.cat([layer.norm1(past_l), q], dim=1)
        y, _ = layer.self_attn(q, kv, kv, attn_mask=None, key_padding_mask=key_padding_mask, need_weights=False)
        x = x + layer.dropout1(y)
        y = layer.linear2(layer.dropout(layer.activation(layer.linear1(layer.norm2(x)))))
        x = x + layer.dropout2(y)
        full_l = x if past_l is None else torch.cat([past_l, x], dim=1)
        full_states.append(full_l.detach() if detach_past_on_return else full_l)
    return x, full_states, keep_full


def _compute_weighted_total_loss(cfg: TrainConfig, terms: Dict[str, torch.Tensor]) -> torch.Tensor:
    loss = cfg.rec_w * terms["loss_rec"]
    loss = loss + cfg.token_ce_w * terms["loss_ce"]
    loss = loss + cfg.norm_match_w * terms["loss_norm_match"]
    loss = loss + cfg.temporal_diff_w * terms["loss_temporal_diff"]
    loss = loss + cfg.gauss_align_w * terms["loss_align"]
    loss = loss + cfg.softptr_w * terms["loss_softptr"]
    loss = loss + cfg.chunk_logit_consistency_w * terms["loss_chunk_logit_consistency"]
    loss = loss + cfg.possharp_w * terms["L_pos"] + cfg.marg_w * terms["L_marg"] + cfg.info_w * terms["L_info"]
    return loss


def _compute_norm_triplet(
    x_hat: torch.Tensor,
    x_pad: torch.Tensor,
    mask_pad: torch.Tensor,
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    eps = 1e-8
    valid = mask_pad.to(x_pad.dtype).unsqueeze(-1)
    den = valid.sum().clamp_min(eps)
    xpad_norm = ((valid * x_pad).detach().norm(dim=-1).sum()) / den
    xhat_norm = ((valid * x_hat).detach().norm(dim=-1).sum()) / den
    return xhat_norm, xpad_norm, (xhat_norm - xpad_norm).abs()


@dataclass
class TrainChunkState:
    token_past: Optional[torch.Tensor] = None
    recon_past: Optional[torch.Tensor] = None
    ...
    tok_ids_chunks: List[torch.Tensor] = None
    tok_logits_chunks: List[torch.Tensor] = None
    tok_emb_chunks: List[torch.Tensor] = None
    rec_chunks: List[torch.Tensor] = None
    sums: Dict[str, float] = None
    counts: Dict[str, int] = None


def _init_train_chunk_state() -> TrainChunkState:
    return TrainChunkState(
        tok_ids_chunks=[],
        tok_logits_chunks=[],
        tok_emb_chunks=[],
        rec_chunks=[],
        sums={
            ...
            "loss_ce": 0.0,
            "loss_norm_match": 0.0,
            "loss_temporal_diff": 0.0,
            "loss_align_p2l": 0.0,
            "loss_align_l2p": 0.0,
            "loss_softptr_p2l": 0.0,
            "loss_softptr_l2p": 0.0,
            "loss_chunk_logit_consistency": 0.0,
            "band_mass_p2l": 0.0,
            "band_mass_l2p": 0.0,
            "L_pos": 0.0,
            "L_marg": 0.0,
        ...
        counts={"tok": 0, "rec": 0, "chunk_cons": 0},
    )


def _accumulate_chunk_terms(state: TrainChunkState, chunk_terms: Dict[str, torch.Tensor]) -> None:
    for k in state.sums.keys():
        state.sums[k] += float(chunk_terms.get(k, torch.zeros(())).detach().item())


def _finalize_train_batch_outputs_and_terms(
    *,
    state: TrainChunkState,
    cfg: TrainConfig,
    model: MotionSeq2SeqARAE,
    device: torch.device,
    batch_size: int,
    ...
    token_ids = torch.cat(state.tok_ids_chunks, dim=1) if len(state.tok_ids_chunks) > 0 else torch.zeros((batch_size, 0), device=device, dtype=torch.long)
    token_logits = torch.cat(state.tok_logits_chunks, dim=1) if len(state.tok_logits_chunks) > 0 else torch.zeros((batch_size, 0, cfg.vocab_size), device=device)
    out = {
        "x_hat": torch.cat(state.rec_chunks, dim=1) if len(state.rec_chunks) > 0 else torch.zeros((batch_size, 0, model.feat_dim), device=device),
        "mask_pad": mask_pad,
        "token_ids": token_ids,
        "token_logits": token_logits,
        "latent_mask": latent_mask,
    ...
    n_tok = max(state.counts["tok"], 1)
    n_rec = max(state.counts["rec"], 1)
    terms = {
        "loss_rec": torch.tensor(state.sums["loss_rec"] / n_rec, device=device),
        "loss_rec_root": torch.tensor(state.sums["loss_rec_root"] / n_rec, device=device),
        "loss_rec_jtr": torch.tensor(state.sums["loss_rec_jtr"] / n_rec, device=device),
        "loss_ce": torch.tensor(state.sums["loss_ce"] / n_tok, device=device),
        "loss_norm_match": torch.tensor(state.sums["loss_norm_match"] / n_rec, device=device),
        "loss_temporal_diff": torch.tensor(state.sums["loss_temporal_diff"] / n_rec, device=device),
        "loss_align_p2l": torch.tensor(state.sums["loss_align_p2l"] / n_tok, device=device),
        "loss_align_l2p": torch.tensor(state.sums["loss_align_l2p"] / n_rec, device=device),
        "loss_softptr_p2l": torch.tensor(state.sums["loss_softptr_p2l"] / n_tok, device=device),
        "loss_softptr_l2p": torch.tensor(state.sums["loss_softptr_l2p"] / n_rec, device=device),
        "loss_chunk_logit_consistency": torch.tensor(state.sums["loss_chunk_logit_consistency"] / max(state.counts["chunk_cons"], 1), device=device),
        "band_mass_p2l": torch.tensor(state.sums["band_mass_p2l"] / n_tok, device=device),
        "band_mass_l2p": torch.tensor(state.sums["band_mass_l2p"] / n_rec, device=device),
        "L_pos": torch.tensor(state.sums["L_pos"] / n_tok, device=device),
        "L_marg": torch.tensor(state.sums["L_marg"] / n_tok, device=device),
        "L_info": torch.tensor(state.sums["L_info"] / n_tok, device=device),
    }
    terms["loss_align"] = 0.5 * (terms["loss_align_p2l"] + terms["loss_align_l2p"])
    terms["loss_softptr"] = 0.5 * (terms["loss_softptr_p2l"] + terms["loss_softptr_l2p"])
    terms["loss"] = _compute_weighted_total_loss(
        cfg,
        {
            "loss_rec": terms["loss_rec"],
            "loss_ce": terms["loss_ce"],
            "loss_norm_match": terms["loss_norm_match"],
            "loss_temporal_diff": terms["loss_temporal_diff"],
            "loss_align": terms["loss_align"],
            "loss_softptr": terms["loss_softptr"],
            "loss_chunk_logit_consistency": terms["loss_chunk_logit_consistency"],
            "L_pos": terms["L_pos"],
            "L_marg": terms["L_marg"],
            "L_info": terms["L_info"],
        },
...
def _build_train_log(
    *,
    epoch: int,
    step: int,
    terms: Dict[str, torch.Tensor],
    xhat_norm: torch.Tensor,
    xpad_norm: torch.Tensor,
    norm_gap: torch.Tensor,
    elapsed_sec: float,
) -> Dict[str, float]:
    return {
        "split": "train",
        ...
        "loss_rec": float(terms["loss_rec"].item()),
        "loss_rec_root": float(terms["loss_rec_root"].item()),
        "loss_rec_jtr": float(terms["loss_rec_jtr"].item()),
        "loss_ce": float(terms["loss_ce"].item()),
        "loss_norm_match": float(terms["loss_norm_match"].item()),
        "loss_temporal_diff": float(terms["loss_temporal_diff"].item()),
        "loss_align": float(terms["loss_align"].item()),
        "loss_align_p2l": float(terms["loss_align_p2l"].item()),
        "loss_align_l2p": float(terms["loss_align_l2p"].item()),
        "loss_softptr": float(terms["loss_softptr"].item()),
        "loss_softptr_p2l": float(terms["loss_softptr_p2l"].item()),
        "loss_softptr_l2p": float(terms["loss_softptr_l2p"].item()),
        "loss_chunk_logit_consistency": float(terms["loss_chunk_logit_consistency"].item()),
        "band_mass_p2l": float(terms["band_mass_p2l"].item()),
        "band_mass_l2p": float(terms["band_mass_l2p"].item()),
        "L_pos": float(terms["L_pos"].item()),
        "L_marg": float(terms["L_marg"].item()),
        "L_info": float(terms["L_info"].item()),
...
def _compute_chunk_total_loss(cfg: TrainConfig, chunk_terms: Dict[str, torch.Tensor]) -> torch.Tensor:
    loss_align = 0.5 * (chunk_terms["loss_align_p2l"] + chunk_terms["loss_align_l2p"])
    loss_softptr = 0.5 * (chunk_terms["loss_softptr_p2l"] + chunk_terms["loss_softptr_l2p"])
    return _compute_weighted_total_loss(
        cfg,
        {
            "loss_rec": chunk_terms["loss_rec"],
            "loss_ce": chunk_terms["loss_ce"],
            "loss_norm_match": chunk_terms["loss_norm_match"],
            "loss_temporal_diff": chunk_terms["loss_temporal_diff"],
            "loss_align": loss_align,
            "loss_softptr": loss_softptr,
            "loss_chunk_logit_consistency": chunk_terms["loss_chunk_logit_consistency"],
            "L_pos": chunk_terms["L_pos"],
            "L_marg": chunk_terms["L_marg"],
            "L_info": chunk_terms["L_info"],
        },
    )


def _backward_chunk_loss(
    *,
    cfg: TrainConfig,
    opt: torch.optim.Optimizer,
    model: MotionSeq2SeqARAE,
    chunk_terms: Dict[str, torch.Tensor],
) -> None:
    loss_chunk = _compute_chunk_total_loss(cfg, chunk_terms)
...
def _compute_p2l_chunk_losses(
    *,
    model: MotionSeq2SeqARAE,
    cfg: TrainConfig,
    sp_cfg: Dict[str, Any],
    proposal: Optional["ProposalConstraintsSoftmax"],
    need_softptr: bool,
    a_len: int,
    a_mask_chunk: torch.Tensor,
    lm_chunk: torch.Tensor,
    attn_c: List[Optional[torch.Tensor]],
    memA_chunk: torch.Tensor,
    hid_c: List[Optional[torch.Tensor]],
    logits_c: torch.Tensor,
    ids_c: torch.Tensor,
    memA_keys: torch.Tensor,
    memA_mask: torch.Tensor,
    t_start: int,
    t_len: int,
    token_past_in: Optional[torch.Tensor],
    attn_apply: str,
    ...
    losses: Dict[str, torch.Tensor] = {
        ...
        "loss_chunk_logit_consistency": torch.zeros((), device=device),
    ...
    if cfg.token_ce_w > 0.0:
        ...
        losses["loss_ce"] = F.cross_entropy(logits_v, ids_v) if logits_v.numel() > 0 else torch.zeros((), device=device)

    attn_local_p2l: List[Optional[torch.Tensor]] = [None if ai is None else ai[:, :, :, -a_len:] for ai in attn_c]
    if cfg.gauss_align_enable:
        losses["loss_align_p2l"] = model.gauss_align_metric(attn_local_p2l, src_keep_mask=a_mask_chunk, tgt_keep_mask=lm_chunk)
        losses["band_mass_p2l"] = model.gauss_align_metric.compute_band_mass(attn_local_p2l, src_keep_mask=a_mask_chunk, tgt_keep_mask=lm_chunk)
    ...
    if need_softptr:
        ...
        if L2 > L1:
            head_topk = int(sp_cfg.get("head_topk", 0))
            head_topk_opt = None if head_topk <= 0 else head_topk
            losses["loss_softptr_p2l"], _ = model.softptr(
                attn_local_p2l,
                memA_chunk,
                hid_c,
    ...
    if cfg.chunk_logit_consistency_enable:
        ...
        losses["loss_chunk_logit_consistency"] = (((logits_c - logits_np) ** 2) * vm).sum() / den
        did_chunk_cons = True

    if proposal is not None:
        losses["L_pos"], losses["L_marg"], losses["L_info"] = proposal(logits_c, valid_mask=lm_chunk)
    return losses, did_chunk_cons


def _compute_l2p_chunk_losses(
    *,
    model: MotionSeq2SeqARAE,
    cfg: TrainConfig,
    sp_cfg: Dict[str, Any],
    need_softptr_l2p: bool,
    t_len: int,
    a_mask_chunk: torch.Tensor,
    lm_chunk: torch.Tensor,
    attn_c_recon: List[Optional[torch.Tensor]],
    memB_chunk: Optional[torch.Tensor],
    hid_c_recon: List[Optional[torch.Tensor]],
    x_hat_chunk: torch.Tensor,
    x_target_chunk: torch.Tensor,
    mask_target_chunk: torch.Tensor,
    mean_t: torch.Tensor,
    std_t: torch.Tensor,
    bridge: VPoserSMPLJointBridge,
) -> Dict[str, torch.Tensor]:
    device = x_hat_chunk.device
    losses: Dict[str, torch.Tensor] = {
        "loss_rec": torch.zeros((), device=device),
        "loss_rec_root": torch.zeros((), device=device),
        "loss_rec_jtr": torch.zeros((), device=device),
        "loss_norm_match": torch.zeros((), device=device),
        "loss_temporal_diff": torch.zeros((), device=device),
    ...
    losses["loss_rec"], losses["loss_rec_root"], losses["loss_rec_jtr"] = recon_loss_vposer_jtr(
        x_hat_chunk,
        x_target_chunk,
        mask_target_chunk,
        mean=mean_t,
        std=std_t,
        bridge=bridge,
        root_w=cfg.rec_root_w,
        joint_w=cfg.rec_joint_w,
    )
    losses["loss_norm_match"] = masked_norm_match_loss(x_hat_chunk, x_target_chunk, mask_target_chunk)
    losses["loss_temporal_diff"] = masked_temporal_diff_mse(x_hat_chunk, x_target_chunk, mask_target_chunk)

    attn_local_l2p: List[Optional[torch.Tensor]] = [None if (ai is None or t_len <= 0) else ai[:, :, :, -t_len:] for ai in attn_c_recon]
    if cfg.gauss_align_enable and t_len > 0:
        l2p_sigma_scale = 1.0 / max(model.compression_ratio, 1e-6)
        losses["loss_align_l2p"] = model.gauss_align_metric(
            attn_local_l2p,
            src_keep_mask=lm_chunk,
            tgt_keep_mask=a_mask_chunk,
            sigma_scale=l2p_sigma_scale,
        )
        losses["band_mass_l2p"] = model.gauss_align_metric.compute_band_mass(
            attn_local_l2p,
            src_keep_mask=lm_chunk,
            tgt_keep_mask=a_mask_chunk,
            sigma_scale=l2p_sigma_scale,
    ...
    if need_softptr_l2p and t_len > 0 and memB_chunk is not None:
        ...
        if L2 > L1:
            head_topk = int(sp_cfg.get("head_topk", 0))
            head_topk_opt = None if head_topk <= 0 else head_topk
            losses["loss_softptr_l2p"], _ = model.softptr(
                attn_local_l2p,
                memB_chunk,
                hid_c_recon,
                src_keep_mask=lm_chunk,
...
def _process_train_chunk(
    *,
    model: MotionSeq2SeqARAE,
    cfg: TrainConfig,
    state: TrainChunkState,
    train_tok_stats: TokenStatsAggregator,
    opt: torch.optim.Optimizer,
    x_pad: torch.Tensor,
    mask_pad: torch.Tensor,
    ...
    memA_chunk, encA_full, memA_mask = _encode_chunk_incremental(
        ...
        h_a,
        a_mask_chunk,
        state.encA_layer_cache,
        state.maskA_cache,
        detach_past_on_return=detach_chunk_pkv_train,
    )
    memA_keys = encA_full[-1]
    chunk_terms: Dict[str, torch.Tensor] = {k: torch.zeros((), device=device) for k in state.sums.keys()}
    ...
    if t_len > 0:
        ...
        token_past_in = state.token_past
        ids_c, logits_c, embs_c, attn_c, hid_c, state.token_past = model.token_decoder.generate(
            memA_keys,
            ~memA_mask,
            t_len,
            tau=cfg.gumbel_tau,
        ...
        state.tok_ids_chunks.append(ids_c.detach())
        state.tok_logits_chunks.append(logits_c.detach())
        state.tok_emb_chunks.append(embs_c.detach())
        state.counts["tok"] += 1
        train_tok_stats.update(logits_c.detach(), ids_c.detach(), lm_chunk.detach())
        p2l_losses, did_chunk_cons = _compute_p2l_chunk_losses(
            model=model,
            cfg=cfg,
            sp_cfg=sp_cfg,
            proposal=proposal,
            need_softptr=need_softptr,
            a_len=a_len,
            a_mask_chunk=a_mask_chunk,
            lm_chunk=lm_chunk,
            attn_c=attn_c,
            memA_chunk=memA_chunk,
            hid_c=hid_c,
            logits_c=logits_c,
            ids_c=ids_c,
            memA_keys=memA_keys,
            ...
            t_start=t_start,
            t_len=t_len,
            token_past_in=token_past_in,
            attn_apply=attn_apply,
        )
        chunk_terms.update(p2l_losses)
        if did_chunk_cons:
            state.counts["chunk_cons"] += 1
    ...
    if memB_chunk is not None:
        ...
        memB_keys = state.memB_cache
        memB_mask = state.maskB_cache
    if memB_keys is None or memB_mask is None:
        state.memA_cache = memA_keys.detach()
        state.maskA_cache = memA_mask
        state.encA_layer_cache = encA_full
        return False

    h_c, attn_c_recon, hid_c_recon, state.recon_past = model.recon_decoder(
        memB_keys,
        ~memB_mask,
        out_len=a_len,
        return_attn=need_attn_recon,
        return_hidden=need_softptr_l2p,
        attn_apply=attn_apply,
        past_inputs=state.recon_past,
        pos_offset=a_start,
        detach_past_on_return=detach_chunk_pkv_train,
    )
    x_patch_hat_c = model.out_proj(h_c)
    x_hat_chunk = unpatchify(x_patch_hat_c, model.patch_len, model.feat_dim)
    state.rec_chunks.append(x_hat_chunk.detach())
    state.counts["rec"] += 1

    l2p_losses = _compute_l2p_chunk_losses(
        model=model,
        cfg=cfg,
        sp_cfg=sp_cfg,
        need_softptr_l2p=need_softptr_l2p,
        t_len=t_len,
        a_mask_chunk=a_mask_chunk,
        lm_chunk=lm_chunk,
        attn_c_recon=attn_c_recon,
        memB_chunk=memB_chunk,
        hid_c_recon=hid_c_recon,
        x_hat_chunk=x_hat_chunk,
        x_target_chunk=x_target_chunk,
        mask_target_chunk=mask_target_chunk,
        mean_t=mean_t,
        std_t=std_t,
        bridge=bridge,
    )
    chunk_terms.update(l2p_losses)
    ...
    _accumulate_chunk_terms(state, chunk_terms)
    state.memA_cache = memA_keys.detach()
    state.maskA_cache = memA_mask
    state.encA_layer_cache = encA_full
    state.memB_cache = memB_keys.detach()
    state.maskB_cache = memB_mask
    if memB_chunk is not None:
        state.encB_layer_cache = encB_full
    return True
...
def run_eval(
    *,
    ...
    for batch in dl_test:
        ...
        loss_rec, loss_rec_root, loss_rec_jtr = recon_loss_vposer_jtr(
            out["x_hat"], x_pad, out["mask_pad"], mean=mean_t, std=std_t, bridge=bridge, root_w=cfg.rec_root_w, joint_w=cfg.rec_joint_w
        )
        ...
        if cfg.token_ce_w > 0.0:
            ...
            loss_ce = torch.zeros((), device=device)
        loss_align_p2l = out.get("gauss_align_loss_p2l", torch.zeros((), device=device)) if cfg.gauss_align_enable else torch.zeros((), device=device)
        loss_align_l2p = out.get("gauss_align_loss_l2p", torch.zeros((), device=device)) if cfg.gauss_align_enable else torch.zeros((), device=device)
        ...
        loss_chunk_cons = out.get("chunk_logit_consistency_loss", torch.zeros((), device=device))
        L_pos, L_marg, L_info = (
            proposal(out["token_logits"], valid_mask=out["latent_mask"], update_state=False)
            if proposal is not None
            else (torch.zeros((), device=device), torch.zeros((), device=device), torch.zeros((), device=device))
        )
        terms = {
            "loss_rec": loss_rec,
            "loss_ce": loss_ce,
            "loss_norm_match": loss_norm_match,
            "loss_temporal_diff": loss_temporal_diff,
            "loss_align": 0.5 * (loss_align_p2l + loss_align_l2p),
            "loss_softptr": 0.5 * (loss_softptr_p2l + loss_softptr_l2p),
            "loss_chunk_logit_consistency": loss_chunk_cons,
            "L_pos": L_pos,
            "L_marg": L_marg,
            "L_info": L_info,
        }
        loss = _compute_weighted_total_loss(cfg, terms)
        xhat_norm, xpad_norm, norm_gap = _compute_norm_triplet(out["x_hat"], x_pad, out["mask_pad"])
        batch_vals = {
            "loss": loss,
            "loss_rec": loss_rec,
            "loss_rec_root": loss_rec_root,
            "loss_rec_jtr": loss_rec_jtr,
            "loss_ce": loss_ce,
            "loss_norm_match": loss_norm_match,
            "loss_temporal_diff": loss_temporal_diff,
            "loss_align": terms["loss_align"],
            "loss_align_p2l": loss_align_p2l,
            "loss_align_l2p": loss_align_l2p,
            "loss_softptr": terms["loss_softptr"],
            "loss_softptr_p2l": loss_softptr_p2l,
            "loss_softptr_l2p": loss_softptr_l2p,
            "loss_chunk_logit_consistency": loss_chunk_cons,
    ...
    stats = {k: v / denom for k, v in sums.items()}
    for k, v in tok_stats.finalize().items():
        stats[f"id_{k}"] = v
    return stats
...
def train(cfg: TrainConfig) -> None:
    ...
    bridge = VPoserSMPLJointBridge(
        vposer_expr_dir=cfg.vposer_expr_dir,
        body_models_root=cfg.body_models_root,
        device=device,
        gender=cfg.body_model_gender,
        num_betas=cfg.body_model_num_betas,
        num_dmpls=cfg.body_model_num_dmpls,
    ).to(device)
    bridge.eval()
    ...
    if not bool(cfg.detach_chunk_pkv):
        print("[train] forcing chunk cache detach for per-chunk optimizer.step() to avoid cross-chunk autograd reuse.", flush=True)
    step = 0
    best = float("inf")
    ...
    for epoch in range(cfg.epochs):
        ...
        for batch in dl_train:
            ...
            ch = int(cfg.chunk_size) if int(cfg.chunk_size) > 0 else int(max(Tb, N))
            ...
            softptr_apply = str(sp_cfg.get("apply", "all")).lower().strip()
            attn_apply = _merge_attn_apply(([gauss_apply] if cfg.gauss_align_enable else []) + ([softptr_apply] if need_softptr else []))
            need_attn_tok = bool(cfg.gauss_align_enable or need_softptr)
            need_attn_recon = bool(cfg.gauss_align_enable or need_softptr_l2p)
            chunk_plans = _build_chunk_plans(n_a=N, n_b=Tb, a_chunk_size=ch, compression_ratio=cfg.compression_ratio)
            state = _init_train_chunk_state()
            for a_start, a_len, t_start, t_len in chunk_plans:
                did_step = _process_train_chunk(
                    model=model,
                    cfg=cfg,
                    state=state,
                    train_tok_stats=train_tok_stats,
                    opt=opt,
                    x_pad=x_pad,
                    mask_pad=mask_pad,
                    x_patch=x_patch,
                    ...
                    need_softptr=need_softptr,
                    need_softptr_l2p=need_softptr_l2p,
                    need_attn_tok=need_attn_tok,
                    need_attn_recon=need_attn_recon,
                    attn_apply=attn_apply,
                    sp_cfg=sp_cfg,
                    mean_t=mean_t,
                    std_t=std_t,
                    bridge=bridge,
                    proposal=proposal,
                    detach_chunk_pkv_train=detach_chunk_pkv_train,
                )
                if did_step:
                    step += 1

            out, terms = _finalize_train_batch_outputs_and_terms(
                state=state,
                cfg=cfg,
                model=model,
                device=device,
            ...
            if step % max(int(cfg.train_log_interval), 1) == 0:
                log = _build_train_log(
                    epoch=epoch,
                    step=step,
                    terms=terms,
                    xhat_norm=xhat_norm,
                ...
                for k, v in train_tok_stats.finalize().items():
                    log[f"id_{k}"] = v
                train_tok_stats = TokenStatsAggregator(cfg.vocab_size)
        ...
        val = run_eval(cfg=cfg, model=model, dl_test=dl_test, device=device, mean_t=mean_t, std_t=std_t, bridge=bridge, proposal=proposal)
...
@torch.no_grad()
def _infer_chunkwise_like_train(
    model: MotionSeq2SeqARAE,
    x: torch.Tensor,
    mask: torch.Tensor,
    *,
    cfg: TrainConfig,
    latent_embed_mode: str,
    deterministic_tokens: bool,
) -> Dict[str, torch.Tensor]:
    x_pad, mask_pad, x_patch, patch_mask = patchify(x, mask, cfg.patch_len)
    B, N, _ = x_patch.shape
    Tb = int(model.latent_len_max)
    ...
    for a_start, a_len, t_start, t_len in chunk_plans:
        ...
        h_a = model.in_proj(a_chunk)
        h_a = model.enc_pos(h_a, offset=int(a_start))
        _, encA_full, memA_mask = _encode_chunk_incremental(
            model.encoder,
            h_a,
            a_mask_chunk,
            encA_layer_cache,
            maskA_cache,
            detach_past_on_return=bool(cfg.detach_chunk_pkv),
        )
        memA_keys = encA_full[-1]
        ...
        if t_len > 0:
            lm_chunk = latent_mask[:, t_start : t_start + t_len]
            ids_c, logits_c, embs_c, _, _, token_past = model.token_decoder.generate(
                memA_keys,
                ~memA_mask,
                t_len,
                tau=cfg.gumbel_tau,
        ...
        h_c, _, _, recon_past = model.recon_decoder(
            memB_keys,
            ~memB_mask,
            out_len=a_len,
            return_attn=False,
    ...
    token_ids = torch.cat(tok_ids_chunks, dim=1) if len(tok_ids_chunks) > 0 else torch.zeros((B, 0), device=x.device, dtype=torch.long)
    token_logits = torch.cat(tok_logits_chunks, dim=1) if len(tok_logits_chunks) > 0 else torch.zeros((B, 0, cfg.vocab_size), device=x.device)
    token_embs = torch.cat(tok_emb_chunks, dim=1) if len(tok_emb_chunks) > 0 else torch.zeros((B, 0, model.d_model), device=x.device)
    x_hat = torch.cat(rec_chunks, dim=1) if len(rec_chunks) > 0 else torch.zeros((B, 0, model.feat_dim), device=x.device)
...
def encode_tokens(data_root: str, ckpt: str, out_dir: str, split: str, batch_size: int, num_workers: int, seed: int, latent_embed_mode: Optional[str] = None) -> None:
    ...
    for batch in tqdm(dl):
        ...
        out = _infer_chunkwise_like_train(
            model=model,
            x=x,
            mask=m,
            cfg=cfg,
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    ap_tr.add_argument("--feat_dim", type=int, default=36,
                       help="Expected input feature dimension. For root4+vposer32 use 36.")
    ...
    ap_tr.add_argument("--vposer_expr_dir", type=str, default="../HumanML3D/human_body_prior/train/V02_05",
                       help="VPoser experiment directory (yaml + snapshots).")
    ap_tr.add_argument("--body_models_root", type=str, default="../HumanML3D/body_models",
                       help="Root directory that contains smplh/ and dmpls/ model files.")
    ap_tr.add_argument("--body_model_gender", type=str, default="male", choices=["male", "female"],
                       help="BodyModel template gender used for joint-space reconstruction loss.")
    ap_tr.add_argument("--body_model_num_betas", type=int, default=10)
    ap_tr.add_argument("--body_model_num_dmpls", type=int, default=8)
    ap_tr.add_argument("--rec_root_w", type=float, default=1.0,
                       help="Weight for root4 direct reconstruction MSE.")
    ap_tr.add_argument("--rec_joint_w", type=float, default=1.0,
                       help="Weight for VPoser->SMPL-H Jtr reconstruction MSE.")
