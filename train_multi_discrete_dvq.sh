...
SAVE_DIR_20FPS_dvq=./experiments/dvq_20FPS
SAVE_DIR_100FPS_dvq=./experiments/dvq_100FPS
...
python motion_tokenizer_humanml3d_multi_discretizers.py train \
  ...
  --save_dir  $SAVE_DIR_20FPS_dvq \
  --normalize \
  --bottleneck dvq \
  --vocab_size 8192 \
  --dvq_slices 4 --dvq_mode sliced \
  --beta 0.25 \
...
python motion_tokenizer_humanml3d_multi_discretizers.py reconstruct \
  --data_root $DATA_ROOT_20FPS \
  --save_dir  $SAVE_DIR_20FPS_dvq \
  --ckpt      $SAVE_DIR_20FPS_dvq/checkpoints/ckpt_best.pt \
  --split test
...
python eval_recon_humanml3d_motiongpt_metrics.py \
  ...
  --data_root  $DATA_ROOT_20FPS \
  --recon_root $SAVE_DIR_20FPS_dvq/recon_out/test \
  --split test \
  --t2m_path   $T2M_DIR \
...
python motion_tokenizer_humanml3d_multi_discretizers.py reconstruct \
  --data_root $DATA_ROOT_100FPS \
  --save_dir  $SAVE_DIR_100FPS_dvq \
  --ckpt      $SAVE_DIR_100FPS_dvq/checkpoints/ckpt_best.pt \
  --split test
...
python eval_recon_humanml3d_motiongpt_metrics.py \
  ...
  --cfg        $MOTIONGPT_DIR/configs/config_h3d_stage3.yaml \
  --data_root  $DATA_ROOT_100FPS \
  --recon_root $SAVE_DIR_100FPS_dvq/recon_out/test \
  --split test \
  --t2m_path   $T2M_DIR \
  --meta_dir   $MOTIONGPT_DIR/assets/meta \
